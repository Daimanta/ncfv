...
#    ncfv - New Command-line File Verify
#
...
from ncfv import caching
from ncfv import cftypes
from ncfv import hash
from ncfv import fileutil
...
class Config:
    ...
    def setx(self, o, v):
        ...
        "dirsort", "cmdlinesort", "ignorecase", "rename", "search", "dereference", "unquote", "private_torrent"):
            self.setbool(o, v)
            ...
            while 1:
                ...
                if len(x) != 2:
                    ...
                    try:
                        ...
                        raise sys.exc_info()[0], "%s:%i: %s" % (filename, l, err), sys.exc_info()[
                            2]  # reuse the traceback of the original exception, but add file and line numbers to the error
...
__homepage__ = 'http://ncfv.sourceforge.net/'
...
class ChksumType:
    ...
    def search_file(self, filename, filecrc, filesize, errfunc, errargs):
        if (not config.search or
                (filesize < 0 and (
                        not filecrc or not config.docrcchecks))):  # don't bother searching if we don't have anything to compare against
            errfunc(*errargs)
            return -2
        alreadyok = None
        ...
        try:
            if fpath:
                if config.ignorecase:
                    ...
                    filename = osutil.path_join(fpath,
                                                filenametail)  # fix the dir the orig filename is in, so that the do_f_found can rename it correctly
            else:
        ...
        try:
            ...
            if config.docrcchecks and filecrc:
                ...
                if c:
                    self.search_file(filename, filecrc, filesize,
                                     self.do_f_badcrc,
                                     (l_filename, 'crc does not match (%s!=%s)' % (filecrct, hexlify(c))))
                    return -2
            else:
                ...
                UnicodeError), a:  # UnicodeError can occur if python can't map the filename to the filesystem's encoding
            self.search_file(filename, filecrc, filesize,
...
class PAR(ChksumType, MD5_MixIn):
    ...
    def do_test_chksumfile(self, file):
        ...
        if version not in (0x00000900,
                           0x00010000):  # ver 0.9 and 1.0 are the same, as far as we care.  Future versions (if any) may very likey have incompatible changes, so don't accept them either.
            raise EnvironmentError, (errno.EINVAL, "can't handle PAR version %s" % ver2str(version))

        view.ev_test_cf_begin('par v%s' % ver2str(version), file.name,
                              'created by %s v%s' % (prog2str(client >> 24), ver2str(client & 0xFFFFFF)))
...
# ---------- .torrent ----------
from ncfv.BitTorrent import bencode, btformats
import sha
...
class Torrent(ChksumType):
        ...
        def init_file(filenameparts, ftotpos, filesize):
            ...
            if not done:
                ...
                if not config.docrcchecks:  # if we aren't testing checksums, just use the standard test_file function, so that -s and such will work.
                    ...
                    return
                filename = self.mangle_filename(filename)
                done = not filenamefilter.should_test(
                    filename)  # if we don't want to test this file, just pretending its done already has the desired effect.
                if not done:
                    stats.num += 1
                try:
                    l_filename = self.find_local_filename(filename)
                    ...
                    if fs != filesize:
                        ...
                        UnicodeError), e:  # UnicodeError can occur if python can't map the filename to the filesystem's encoding
                    if not done:
            ...
            if config.strippaths == 0:
                ...
                try:
                    dirname = cffndecode(dirname, encoding)
                        ...
                        FilenameError), e:  # lookup error is raised when specified encoding isn't found.
                    stats.cferror += 1
    ...
    def make_chksumfile_finish(self, file):
        ...
        data = {'info': info, 'announce': cfencode(config.announceurl.strip(), 'UTF-8'),
...
class SFV_Base(TextChksumType):
    ...
    def make_chksumfile_create(self, filename):
        ...
        file.write('; Generated by ncfv v%s on %s' % (
        __version__, time.strftime('%Y-%m-%d at %H:%M.%S', time.gmtime(time.time()))) + os.linesep + ';' + os.linesep)
        return file
...
class CSV4(TextChksumType, CRC_MixIn):
    ...
    @staticmethod
    def auto_chksumfile_match(file, _autorem=re.compile(
        r'%s[0-9]+,[0-9a-fA-F]{8},%s[\n\r]*$' % (_csvfnautore, _csvstrautore))):
        return _autorem.match(file.peekline(4096)) is not None
    ...
    def do_test_chksumline(self, l):
        ...
        self.test_file(osutil.path_join(self.fixpath(path), name), unhexlify(x.group(4)),
                       int(x.group(3)))  # we need to fixpath before path.join since osutil.path_join looks for path.sep
...
class JPEGSheriff_CRC(TextChksumType, CRC_MixIn):
    ...
    @staticmethod
    def auto_chksumfile_match(file, _autorem=re.compile(r'^Filename\s+(Filesize\s+)?.*?CRC-?32.*^-+(\s+-+){1,4}\s*$',
    ...
    def do_test_chksumfile_print_testingline(self, file):
        x = re.search(r'^Filename\s+(Filesize\s+)?(.+?)??CRC-?32.*^-+(\s+-+){1,4}\s*$', file.peekdecoded(1024),
                      re.DOTALL | re.IGNORECASE | re.MULTILINE)
    ...
    def make_chksumfile_create(self, filename):
        file = TextChksumType.make_chksumfile_create(self, filename)
        file.write('Generated by: ncfv (v%s)' % (__version__) + os.linesep)
    ...
    if S_ISDIR(st.st_mode):
        ...
        if config.dereference and st.st_ino:
            ...
            if dir_key in _visited_dirs:
                if noisy:
                    view.ev_generic_warning(
                        "skipping already visited dir %s %s" % (view.perhaps_showpath(name), dir_key))
                return 0
...
def make(cftype, ifilename, testfiles):
    ...
    if ifilename:
        ...
        if config.gzip == 1 and filename[
                                -3:] != '.gz':  # if user does -zz, perhaps they want to force the filename to be kept?
            filename += '.gz'
...
atrem = re.compile(r'md5|sha1|\.(csv|sfv|par|p[0-9][0-9]|par2|torrent|crc)(\.gz)?$',
                   re.IGNORECASE)  # md5sum/sha1sum files have no standard extension, so just search for files with md5/sha1 in the name anywhere, and let the test func see if it really is one.
...
def printusage(err=0):
    ...
    phelp('Usage: ncfv [opts] [-p dir] [-T|-C] [-t type] [-f file] [files...]')
    ...
    phelp(' --version show ncfv and module versions')
...
def printhelp():
    view.pinfo('ncfv v%s - Copyright (C) 2000-2005 Matthew Mueller - GPL license' % __version__)
    printusage()
...
def main(argv=None):
    ...
    try:
        optlist, args = getopt.getopt(argv, 'rRlLTCt:f:mMnNsSp:uUiIvVzZqQh?',
                                      ['list=', 'list0=', 'fixpaths=', 'strippaths=', 'showpaths=', 'renameformat=',
                                       'progress=', 'unquote=', 'help', 'version',
                                       'encoding=',
        ...
        for o, a in optlist:
            ...
            if o == '-T':
                ...
                if a != 'auto' and not cftypes.has_handler(a):
                    view.perror('ncfv: type %s not recognized' % a)
                ...
                try:
                    ...
                    pass
                try: print
                'fchksum %s' % fchksum.version()
                except NameError:
                pass
                print
                'python %08x-%s' % (sys.hexversion, sys.platform)
                sys.exit(0)
            prevopt = o
    except CFVValueError, e:
        view.perror('ncfv: %s' % e)
        sys.exit(1)
