...
def _getfilechecksum(filename, hasher, callback):
    ...
    if f == sys.stdin or _nommap or callback:
        ...
        try:
            ...
            mmapsize = min(s,
                           _FALLBACK_MMAP)  # mmap size is limited by C's int type, which even on 64 bit arches is often 32 bits, so we can't use sys.maxint either.  If we get the error, just assume 32 bits.
            m = hasher(dommap(f.fileno(), mmapsize))
            f.seek(mmapsize)
            return finish(m,
                          mmapsize)  # unfortunatly, python's mmap module doesn't support the offset parameter, so we just have to do the rest of the file the old fashioned way.
        return m.digest(), s
