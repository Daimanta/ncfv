...
def cfv_stdin_test(cmd, file):
    ...
    try:
        ...
        x2 = re.search('^' + re.escape(x.group(1)) + '[\t ]*' + re.escape(
        ...
        try:
            ...
            if t != 'csv2':  # csv2 has only filesize, hence checksum never happens, so no progress
                x = re.match(re.escape(file) + r' : (\.{20}[-\b.#\\|/]*)[ \r\n]+' + '\x1b\\[K' + re.escape(
                    cf1) + ': (\d+) files, (\d+) OK.  [\d.]+ seconds, [\d.]+K(/s)?$', o1, re.M | re.DOTALL)
                if not x: raise cst_err(4)
            x2 = re.match(r' : (\.[-\b.#/|\\]*)[\t ]*[ \r\n]+' + '\x1b\\[K' + re.escape(
                cf2) + ': (\d+) files, (\d+) OK.  [\d.]+ seconds, [\d.]+K(/s)?$', o2, re.M)
            if not x2: raise cst_err(5)
            if t == 'crc':
            ...
            r = er
        test_log_results('progress=yes stdin/out of ' + t + ' with file ' + file, (s1, s2),
                         o1 + '\n' + o2 + '\n--\n' + c1 + '\n' + c2, r, None)
    finally:
        shutil.rmtree(dir)
...
rx_StatusLine = rx_Begin + ''.join(
    map(optionalize, [rx_badcrc, rx_badsize, rx_notfound, rx_ferror, rx_unv, rx_cferror, rx_misnamed])) + rx_End
...
class OneOf:
    ...
    def __cmp__(self, a):
        ...
        return (a > self.possible[0]) - (a < self.possible[0])
...
def cfv_substatus_test(s, o, unv=0, notfound=0, badcrc=0, badsize=0, cferror=0, ferror=0):
    expected_status = (badcrc and 2) | (badsize and 4) | (notfound and 8) | (ferror and 16) | (unv and 32) | (
            cferror and 64)
...
def cfv_all_test(s, o, files=-2, ok=0, unv=0, notfound=0, badcrc=0, badsize=0, cferror=0, ferror=0, misnamed=0):
    ...
    if x:
        if files == -2:
            files = functools.reduce(operator.add, [ok, badcrc, badsize, notfound, ferror])
        ...
        if not filter(icomp, map(None, expected, actual)):
            sresult = cfv_status_test(s, o, unv=unv, notfound=notfound, badcrc=badcrc, badsize=badsize, cferror=cferror,
                                      ferror=ferror)
            if sresult:
                return sresult
            return 0
...
def cfv_listdata_abs_test(s, o):
    if s == 0 and re.search('^' + re.escape('\0'.join(map(joincurpath, ['data1', 'data2', 'data3', 'data4']))) + '\0$',
                            o, re.I):
        return 0
    return 1
...
def T_test(f, extra=None):
    ...
    test_generic(cmd + " -v -T --list0=unverified -f test" + f + " unchecked.dat testfix.csv data1",
                 cfv_listdata_unv_test, stderr="/dev/null")
...
def create_funkynames(t, d, chr, deep):
    ...
    for i in range(1, 256):
        ...
        if n in (os.sep, os.altsep):
            continue
        if fmt_istext(t) and len((
                                         'a' + n + 'a').splitlines()) > 1:  # if n is a line separator (note that in unicode, this is more than just \r and \n)
            continue
...
def C_funkynames_test(t):
    ...
    for deep in (0, 1):
        ...
        try:
            ...
            numencodable = len(filter(is_fmtencodable, os.listdir(d)))
            ...
            test_generic(cfvcmd + " -v -u -T -p %s -f %s" % (d, cfn),
            ...
            test_generic(cfvcmd + " -v --encoding=utf-8 -T -p %s -f %s" % (d, cfn),
                         rcurry(cfv_all_test, files=num, ok=num))
            test_generic(cfvcmd + " -v --encoding=utf-8 -u -T -p %s -f %s" % (d, cfn),
                         rcurry(cfv_all_test, files=num, ok=num, unv=0))
        finally:
            shutil.rmtree(d)
        ...
        try:
            ...
            test_generic(cfvcmd + " -v -u -T -p %s -f %s" % (d3, dcfn), rcurry(cfv_substatus_test, cferror=1,
                                                                               unv=1))  # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable,unv=numundecodable))
            test_generic(cfvcmd + " -v -m -T -p %s -f %s" % (d3, dcfn), rcurry(cfv_substatus_test,
                                                                               cferror=1))  # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable))
            test_generic(cfvcmd + " -v -m -u -T -p %s -f %s" % (d3, dcfn), rcurry(cfv_substatus_test, cferror=1,
                                                                                  unv=1))  # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable,unv=numundecodable))
            ...
            if not deep:
                ...
                if fmt_hassize(t):
                    test_generic(cfvcmd + " --encoding=raw -v -m -s -T -p %s -f %s" % (d3, dcfn),
                                 rcurry(cfv_all_test, ok=cnum, misnamed=numrenamed))
                ...
                test_generic(cfvcmd + "%s -v -C -p %s -t %s -f %s" % (deep and ' -rr' or '', d3, t, dcfn),
                             rcurry(cfv_all_test, ok=okcnum, ferror=numerr))
...
def search_test(t, test_nocrc=0, extra=None):
    ...
    try:
        ...
        try:
            ...
            print
            'rename of open file in read-only dir worked?  skipping this test.'
    ...
    try:
        ...
        if hassize:
            ...
            experrs = {'badcrc': 1}
        test_generic(cmd + " -v -uu -s -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, files=4, ok=1, misnamed=0, notfound=2, unv=0, **experrs))
        test_generic(cmd + " -v -uu -s -n -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, files=4, ok=1, misnamed=0, notfound=2, unv=1, **experrs))
    finally:
        shutil.rmtree(d)
    ...
    if fmt_cancreate(t):
        ...
        try:
            ...
            test_generic(cmd + " -i -v -s -n -T -p %s -f %s" % (d, dcfn),
    ...
    if fmt_cancreate(t) and hassize:
        ...
        try:
            ...
            test_generic(cmd + " -v -m -T -p %s -f %s" % (d, dcfn),
                         dont_find_dir_test)  # test not finding non-file things in normal mode
            test_generic(cmd + " -v -m -s -T -p %s -f %s" % (d, dcfn),
                         dont_find_dir_test)  # test not finding non-file things in search mode
        finally:
            shutil.rmtree(d)
...
"aa4/bb4/data4" FA323C6D""")
        ...
        test_generic(cfvcmd + r" -v --unquote=yes --strippaths=0 --fixpaths \\/ -s -T -p " + d,
                     rcurry(cfv_all_test, ok=8, misnamed=8))
    finally:
        shutil.rmtree(d)
            ...
            if fmt_istext(t):
                ...
                for enc in utfencodings:
                    ...
                    try:
                        ...
                        test_generic(cfvcmd + " -T -p %s -t %s -f %s" % (datad, t, bommedcfn),
                                     rcurry(cfv_all_test, ok=fnok))
    ...
    try:
        ...
        if files:
            ...
            if files_fnok == len(datafns):
                test_generic(cfvcmd + " -v -T -p " + d + " " + ' '.join(files),
                             rcurry(cfv_all_test, ok=files_fnok, notfound=files_fnerrs))
            test_generic(cfvcmd + " --encoding=raw -v -m -T -p " + d + " " + ' '.join(files),
                         rcurry(cfv_all_test, ok=raw_files_fnok))
            if raw_files_fnok == len(datafns):
                test_generic(cfvcmd + " --encoding=raw -v -T -p " + d + " " + ' '.join(files),
                             rcurry(cfv_all_test, ok=raw_files_fnok))
        ...
        if raw_fnok == len(datafns):
            test_generic(cfvcmd + " --encoding=raw -v -T -p " + d,
                         rcurry(cfv_all_test, ok=raw_fnok, notfound=raw_fnerrs))
            test_generic(cfvcmd + " --encoding=raw -v -u -T -p " + d,
                         rcurry(cfv_all_test, ok=raw_fnok, unv=fnok - raw_fnok, notfound=raw_fnerrs))
    except:
...
def manyfiles_test(t):
    ...
    if not run_long_tests and max_open > 4096:
        print
        'max open files is big (%i)' % max_open,
        max_open = 4096
        print
        'clipping to %i.  Use --full to try the real value' % max_open
    num = max_open + 1
    d = tempfile.mkdtemp()
    try:
...
def show_help_and_exit(err=None):
    ...
    print
    ' --unit  run only unittests, no integration tests'
    print
    ' --help  show this help'
    print
    print
...
def all_tests():
    ...
    test_generic(
        cfvcmd + r" --strippaths=0 --unquote=yes --fixpaths \\/ -T -f testdrivestripquoted.md5 data1 data3 data4",
    ...
    test_generic(cfvcmd + " -i --unquote=yes -T -f testquotedcase.sfv DaTa1 " + os.path.join('a', 'C', 'Foo.bar'),
                 rcurry(cfv_all_test, ok=2))
    ...
    C_test("bsdmd5",
           "-t bsdmd5")  # ,verify=lambda f: test_generic("md5 -c "+f,status_test)) #bsd md5 seems to have no way to check, only create
        ...
        if fmt_istext(t):
            ...
            if t == 'par':
                try:
                    ...
                    nf = 4;
                    err = 0
                test_generic(cfvcmd + " --encoding=utf-16be -T -f test." + t,
                ...
                test_generic(cfvcmd + " --encoding=cp500 -T -f test." + t,
                             rcurry(cfv_all_test, notfound=nf, ferror=err))
                test_generic(cfvcmd + " --encoding=cp500 -i -T -f test." + t,
                             rcurry(cfv_all_test, notfound=nf, ferror=err))
    ...
    for strip in (0, 1):
        test_generic(cfvcmd + " -T --strippaths=%s -p foo -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
                     rcurry(cfv_all_test, ok=7))
        test_generic(cfvcmd + " -T --strippaths=%s -p foo2err -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
                     rcurry(cfv_all_test, ok=4, badcrc=3))
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2err -f %s foo1 foo4" % (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=0, badcrc=2))
        test_generic(cfvcmd + " -T --strippaths=%s -p foo2err1 -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
                     rcurry(cfv_all_test, ok=6, badcrc=1))
        ...
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2badsize -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=5, badsize=1, badcrc=1))
        ...
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2missing -f %s" % (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=4, badcrc=2, notfound=1))
    ...
    try:
        ...
        test_log_results("non-creation of empty torrent on missing announceurl?", '', repr(os.listdir(d)),
                         len(os.listdir(d)) > 1, {})
    finally:
        shutil.rmtree(d)
