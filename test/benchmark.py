...
def run_cfv(args, verbose):
    if verbose >= 2:
        print('running ncfv', args)
    s, o = runcfv(args)
    if s or verbose >= 3:
        print('ncfv returned', s, ', output:')
        print(o)
    if s:
...
def run_create_test(cftype, output_root, input_root, verbose):
    output_fn = os.path.join(output_root, "create_test.%s.%s" % (random.randint(0, 2**31 - 1), cftype))
...
def run(args):
    ...
    times = timeit.repeat(partial(run_create_test, args.type, output_root, input_root, args.verbose),
    ...
    times = timeit.repeat(partial(run_test_test, args.type, cfname, input_root, 1, args.verbose), repeat=args.repeats,
                          number=args.iterations)
    print_times('test', times, args.iterations, args.verbose)

    if args.multitest > 1:
        times = timeit.repeat(partial(run_test_test, args.type, cfname, input_root, args.multitest, args.verbose),
                              repeat=args.repeats, number=args.iterations)
...
def main():
    ...
    create_parser.add_argument('--branch-factor', type=human_int,
                               help='(max) number of files or directories at each level')
